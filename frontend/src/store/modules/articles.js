...
export default {
  ...
  actions: {
    ...
    fetchCommunity({ commit, getters }, communityPk) {
      axios({
        url: drf.articles.community(communityPk),
        method: 'get',
        headers: getters.authHeader
      })
        .then(res => {
    ...
    updateArticle({ commit, getters }, { pk, community_id, title, content}) {
      /* 게시글 수정
      ...
      */
      axios({
        ...
        method: 'put',
        data: { community_id, title, content },
        headers: getters.authHeader,
      })
        .then(res => {
          commit('SET_ARTICLE', res.data)
